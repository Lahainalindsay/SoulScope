...
export default function AdminFeedbackPage() {
  ...
  const [preferenceRows, setPreferenceRows] = useState<StoryPreferenceRow[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setPreferenceRows((preferenceData ?? []) as StoryPreferenceRow[]);
    };
  ...
  const writtenByPage = useMemo(() => {
    return feedbackRows.reduce<Record<string, FeedbackRow[]>>((acc, row) => {
  ...
  return (
    <>
      <Head>
        <title>Admin Feedback | SoulScope</title>
            ...
            <>
              <section className={styles.metricsGrid}>
                <article className={styles.metricCard}>
                  <span className={styles.metricLabel}>Average graphics</span>
                  <strong className={styles.metricValue}>{average(feedbackRows, (row) => row.graphics_rating)}</strong>
                </article>
                <article className={styles.metricCard}>
                  <span className={styles.metricLabel}>Average wording</span>
                  <strong className={styles.metricValue}>{average(feedbackRows, (row) => row.wording_rating)}</strong>
                </article>
                <article className={styles.metricCard}>
                  <span className={styles.metricLabel}>Average clarity</span>
                  <strong className={styles.metricValue}>{average(feedbackRows, (row) => row.clarity_rating)}</strong>
                </article>
                <article className={styles.metricCard}>
                  <span className={styles.metricLabel}>Recent rows</span>
                  <strong className={styles.metricValue}>{feedbackRows.length}</strong>
                </article>
              </section>

              <section className={styles.panel}>
                <h2 className={styles.sectionTitle}>Selected Narrative Styles</h2>
                <div className={styles.countGrid}>
                  {Object.entries(styleCounts).map(([style, count]) => (
                    <div key={style} className={styles.countItem}>
                      <span>{style}</span>
                      <strong>{count}</strong>
                    </div>
                  ))}
              ...
              <section className={styles.panel}>
                <h2 className={styles.sectionTitle}>Written Feedback by Page</h2>
                <div className={styles.feedbackGroups}>
                  {Object.entries(writtenByPage).map(([page, rows]) => (
                    <article key={page} className={styles.feedbackGroup}>
                      <h3>{page}</h3>
                      {rows.slice(0, 8).map((row) => (
                        <div key={row.id ?? `${row.created_at}-${row.scan_id}`} className={styles.feedbackText}>
                          {row.confusing_feedback ? <p><strong>Confusing:</strong> {row.confusing_feedback}</p> : null}
                          {row.change_or_add_feedback ? <p><strong>Change/add:</strong> {row.change_or_add_feedback}</p> : null}
                        </div>
                      ))}
                    </article>
                  ))}
                  {!Object.keys(writtenByPage).length ? <p className={styles.empty}>No written feedback yet.</p> : null}
                </div>
              </section>

              <section className={styles.panel}>
                <h2 className={styles.sectionTitle}>Recent Feedback Rows</h2>
                <div className={styles.tableWrap}>
                  <table className={styles.table}>
                    ...
                    <tbody>
                      {feedbackRows.slice(0, 40).map((row) => (
                        <tr key={row.id ?? `${row.created_at}-${row.scan_id}`}>
                          <td>{row.created_at ? new Date(row.created_at).toLocaleString() : "Unknown"}</td>
                          <td>{row.page ?? "unknown"}</td>
                          <td>{row.graphics_rating ?? "-"}</td>
                          <td>{row.wording_rating ?? "-"}</td>
                          <td>{row.clarity_rating ?? "-"}</td>
                          <td>{row.selected_summary_style ?? "-"}</td>
                          <td>{row.scan_id ?? "-"}</td>
                        </tr>
                      ))}
                    </tbody>
