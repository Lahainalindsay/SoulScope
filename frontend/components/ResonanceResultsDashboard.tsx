...
function buildNarrative(report: SoulScopeReport): NarrativeProfile {
  ...
  const signal = concentrated
    ? `Your voice signal gathered strongly around ${noteMeaning(first?.note)}${second ? ` with a second layer of ${noteMeaning(second.note)}` : ""}. That suggests the scan was not evenly spread; certain parts of your system were speaking louder than others.`
    : `Your voice signal was more spread out across the scan, which suggests a less narrowly concentrated state and more than one part of you participating in the result.`;
  const meaning = low?.score !== undefined && low.score < 35
    ...
    : "This reflection is reading the relationship between what is active, what is steady, and what is asking for more support.";

  return {
    story: [opening, signal, meaning],
    patternTitle: concentrated ? "Concentrated Signal, Uneven Support" : "A Mixed System Looking for Coherence",
    patternSignal: concentrated
      ? "One part of your system appears more pronounced than the rest. The insight should follow that strongest signal while still noticing what is quieter in the background."
      : "The pattern is not a fixed label. It is the relationship between what is available, what is quieter, and how much support the scan suggests you may need.",
    ...
    strength: top ? `What is supporting you in this scan is ${softName(top.title)}. That area remained the most available signal.` : "This scan produced a readable signal.",
    balancePoint: low ? `What deserves attention first is supporting ${softName(low.title)} with one small, clear action instead of pushing harder from the strongest part of you.` : "What deserves attention first is one small action that creates more steadiness.",
    cameraInsight,
  };
}
...
export default function ResonanceResultsDashboard({ report, hiddenNotes = [], onSelectStory, selectedStoryStyle = null }: ResonanceResultsDashboardProps) {
  ...
  return (
    <section className={styles.section}>
      <section className={styles.heroCard}><div className={styles.heroCopy}><p className={styles.eyebrow}>Latest Reflection</p><h1 className={styles.title}>{report.primaryPattern.name}</h1>{narrative.story.map((line) => <p key={line} className={styles.noteText}>{line}</p>)}{narrative.cameraInsight ? <p className={styles.noteText}>{narrative.cameraInsight}</p> : null}</div></section>
      <section className={styles.mapSection}><div className={styles.mapFrame}><NoteAuraMap noteEnergies={visibleEnergies} title="Resonance Map" /></div></section>
      <section className={styles.patternStrip}><article className={styles.patternCard}><p className={styles.noteStatus}>What this may reflect right now</p><h3 className={styles.patternTitle}>{narrative.patternTitle}</h3><p className={styles.patternTheme}>{narrative.patternSignal}</p></article><article className={styles.patternCard}><p className={styles.noteStatus}>What this may feel like</p><p className={styles.patternTheme}>{narrative.feelsLike}</p></article></section>
      <section className={styles.patternStrip}><article className={styles.patternCard}><p className={styles.noteStatus}>What is supporting the system</p><p className={styles.patternTheme}>{narrative.strength}</p></article><article className={styles.patternCard}><p className={styles.noteStatus}>What deserves attention first</p><p className={styles.patternTheme}>{narrative.balancePoint}</p></article></section>
      <section className={styles.notesSection}>
        <div className={styles.notesHeader}><div><p className={styles.eyebrow}>Summary Style</p><h2 className={styles.mapTitle}>Choose the version that reads clearest</h2><p className={styles.lead}>All three versions use the same scan. Select the wording that helps you understand the pattern quickly.</p></div></div>
        <div className={styles.topNotesGrid}>{report.storyCandidates.map((candidate) => { const isSelected = candidate.style === selectedStoryStyle; return <article key={candidate.style} className={styles.noteCard}><div className={styles.noteTop}><p className={styles.noteStatus}>{candidate.style}</p><button type="button" className={styles.breakdownButton} onClick={() => onSelectStory?.(candidate.style)} aria-pressed={isSelected}>{isSelected ? "Selected" : "Select Version"}</button></div><h3 className={styles.noteName}>{candidate.title}</h3><p className={styles.noteTheme}>{candidate.summary}</p></article>; })}</div>
      </section>
      <details className={styles.technicalDetails}><summary className={styles.technicalSummary}>View Signal Details</summary><div className={styles.technicalBody}><p className={styles.technicalIntro}>This layer stays collapsed so the insight remains centered on the human pattern.</p><div className={styles.technicalGrid}><article className={styles.technicalCard}><p className={styles.sectionLabel}>Primary Pattern</p><p className={styles.domainPattern}>{report.primaryPattern.name}</p><p className={styles.domainPattern}>Confidence {Math.round(report.primaryPattern.confidence * 100)}%</p></article><article className={styles.technicalCard}><p className={styles.sectionLabel}>Domain Signals</p><ul className={styles.technicalList}>{domains.map((domain) => <li key={domain.title}>{domain.title}: {Math.round(domain.score)} ({domain.functionalState})</li>)}</ul></article></div></div></details>
    </section>
  );
}
